...
IR_Sensor irleft = IR_Sensor(A0);
IR_Sensor irright = IR_Sensor(A1);

void setup() {
  Serial.begin(115200);//37500
  for(int i = 0; i < 4; i++) {
      drivers[i].init(); // initialize PID
      drivers[i].setCountsPerRad(83046.4); // set encoder counts per radian of output shaft
      drivers[i].setTorqueConstant(4.331); // set motor Nm/A at the output shaft
      drivers[i].setCommandCurrentMax(5.0); // current commands are scaled from 0 to this parameter (amps)
      drivers[i].setCurrentLimit(5.0); // limits absolute value of current command (amps)
      drivers[i].setMaxAngle(0.50); // if motor is past this angle & torque is positive, torque defaults to 0 (rad)
      drivers[i].setMinAngle(-0.50); // if motor is past this angle & torque is negative, torque defaults to 0 (rad)
  }
  irleft.init();
  irright.init();
}
